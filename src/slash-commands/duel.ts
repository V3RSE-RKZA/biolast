	...
	async run (ctx: CommandContext): Promise<void> {
		...
		try {
			...
			const runTurn = () => new Promise<void>((resolve, reject) => {
				const turnCollector = this.app.componentCollector.createCollector(botMessage.id, i => i.user.id === ctx.user.id || i.user.id === member.id, 60000)
						...
						if (playerChoice) {
							...
							await actionCtx.send({
								...
								content: `${icons.danger} You cannot change your action after you have already selected one.` +
									' You must complete your chosen action within **60 seconds** or your turn will be skipped.'
							})
						}
	...
	getDuelEmbed (
		...
		const duelEmb = new Embed()
			...
			.addField(`${player2.user.username}#${player2.user.discriminator} (Level ${player2Data.level})`,
				...
				true)
			.setFooter(`Turn #${turnNumber} / 20 max · 60 seconds to make selection`)
